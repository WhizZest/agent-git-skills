...
> ✅ 创建新分支前先执行 `git fetch` 检查本地是否落后
> ✅ 发现落后时先执行 `git pull --rebase` 再操作
>
> 无远端仓库（纯本地仓库）时跳过此步骤。典型错误场景：本地 main 已落后远端多个 commit 就直接创建新分支，导致分支基于过时代码。`git pull --rebase` 保持线性历史，避免多余的 merge commit。
