...
- 同一操作在多个调用点的写法是否一致？（如多个 CLI 命令获取 bookId 的方式）
- 是否存在可提取为共享函数的重复模式？
- 新增的导出函数是否在所有调用方都被正确使用？
- 不同文件中对同一概念的处理是否有遗漏？（如 A 文件做了归一化但 B 文件没做）
...
- 画出数据流：`用户输入 → 函数A → 归一化/转换 → 函数B → 使用`
- 检查每条路径上的转换是否正确
- 是否存在某些路径跳过了必要的处理步骤？
- 不同路径产出的数据格式是否一致？

示例：审查 `normalizeBookId` 时，追踪所有获取 bookId 的路径：
...
- 每个 return 语句返回的值是否满足声明的类型/格式？
- 如果 JSDoc 说返回 `encodeId 格式`，是否所有路径都确保了这个格式？
- 是否存在「文档说返回 A，但某条路径返回了 B」的情况？
...
- 归一化/校验是否在最外层完成，而非依赖内层函数各自处理？
- 如果内层函数已经做了处理，外层是否还有冗余的防御？
- 推荐模式：在最外层统一归一化，无论数据来自哪个路径。例如 `normalize(input || extract(target))` 而非在内层函数中各自处理
...
- 无 SQL 注入漏洞
- 无 XSS 漏洞
- 库函数对异常输入有防御（null/undefined/空白/类型转换）
- 归一化/校验在最外层完成（防御深度）
